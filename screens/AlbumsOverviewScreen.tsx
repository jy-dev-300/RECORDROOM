...
function getFocusedSectionFrame(layout: AlbumWorldLayout): SectionFrame {
...
function AnimatedSection({
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      position: "absolute",
      left: interpolate(active, [0, 1], [baseFrame.left, focusedFrame.left]),
      top: interpolate(active, [0, 1], [baseFrame.top, focusedFrame.top]),
      width: interpolate(active, [0, 1], [baseFrame.width, focusedFrame.width]),
      height: interpolate(active, [0, 1], [baseFrame.height, focusedFrame.height]),
      zIndex: isFocusedSection ? 20 : 1,
      opacity: dimmed ? interpolate(overlayOpacity.value, [0, 1], [1, 0]) : 1,
    };
  });
